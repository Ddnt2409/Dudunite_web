...
export default function FluxCx({ setTela }) {
  ...
  const [ate, setAte] = useState(
    new Date(ano, mes, 1).toISOString().slice(0, 10)
  );
  const meses = [
    "janeiro",
    "fevereiro",
  ...
  // visibilidade do bloco de CAIXA
  ...
  // ===== Fechamento =====
  const [diaFechar, setDiaFechar] = useState(
  ...
  function assinarListas() {
    ...
    if (modo === "mes") {
      unsubCx.current = listenCaixaDiario(ano, mes, ({ linhas }) =>
        setCxLinhas(linhas)
      );
      ...
      unsubCx.current = listenCaixaDiarioRange(ini, fim, ({ linhas }) =>
        setCxLinhas(linhas)
      );
  ...
  // ===== Extrato agrupado por dia com saldos (banco) =====
  ...
  // ===== Checkbox Realizado (marca/desmarca) =====
  async function onToggleRealizado(linha, checked) {
    try {
      if (linha.origem === "Previsto" && checked) {
        ...
        // volta para Previsto para a MESMA data (mantém no mesmo dia)
        await atualizarFluxo(linha.id, {
  ...
  function badgeStatus(l) {
    ...
    // atrasado: decide rótulo conforme sinal
    if (Number(l.valor) < 0)
      return <span className="badge-bad">ATRASADO</span>;
    ...
    <>
      ...
      <main className="fluxcx-main" style={{ padding: 10 }}>
        ...
        <div className="extrato-card" style={{ marginBottom: 10 }}>
          <div className="extrato-actions" style={{ gap: 8, flexWrap: "wrap" }}>
            ...
            <input
              type="number"
              value={saldoIniCx}
              onChange={(e) => setSaldoIniCx(e.target.value)}
              style={{ width: 90 }}
            />
            ...
            <input
              type="number"
              value={saldoIniBk}
              onChange={(e) => setSaldoIniBk(e.target.value)}
              style={{ width: 90 }}
            />
        ...
        <section className="extrato-card">
          <div className="fluxcx-header" style={{ marginBottom: 6 }}>
            ...
            >
              ...
              {cxSemSaldo && !forcarMostrarCx && (
                <button className="btn-acao" onClick={() => setForcarMostrarCx(true)}>
                  Mostrar
                </button>
              )}
              {cxSemSaldo && forcarMostrarCx && (
                <button className="btn-acao" onClick={() => setForcarMostrarCx(false)}>
                  Ocultar
                </button>
              )}
            </div>
          ...
          {/* quando SEM saldo no caixa, fica oculto; dá pra mostrar manualmente */}
          {!mostrarCaixa ? (
            <div className="cx-oculto">
              <div className="saldo-big">
                Saldo atual do caixa:{" "}
                <span style={{ fontWeight: 900 }}>{money(cxSaldoFinal)}</span>
              </div>
              <button className="btn-acao" onClick={() => setForcarMostrarCx(true)}>
                Mostrar
              </button>
              ...
              >
                <label>
                  ...
                  <input
                    type="date"
                    value={diaFechar}
                    onChange={(e) => setDiaFechar(e.target.value)}
                  />
                </label>
                <label>
                  Data no banco:{" "}
                ...
                <label>
                  ...
                  <input
                    ...
                    step="0.01"
                    value={valorFechar}
                    onChange={(e) => setValorFechar(e.target.value)}
                    style={{ width: 120 }}
                  />
                </label>
                ...
                <table className="extrato">
                  <thead>
                    <tr>
                      ...
                      <th className="th-right" style={{ minWidth: 120 }}>
                        Valor
                      </th>
                      <th className="th-right" style={{ minWidth: 120 }}>
                        Saldo
        ...
        <section className="extrato-card">
          <div className="fluxcx-header" style={{ marginBottom: 6 }}>
            ...
            <div
              style={{ marginLeft: "auto", display: "flex", gap: 14, alignItems: "center" }}
            >
              <span>
              ...
              <span>
                Saldo (Real − Prev): <b>{money(saldoBancoVsPrev)}</b>
              </span>
            </div>
          </div>
              ...
              <div style={{ overflowX: "auto" }}>
                <table className="extrato">
                  <thead>
                    <tr>
                      <th>Descrição</th>
                      <th style={{ minWidth: 120 }}>Forma / Realizado</th>
                      <th style={{ minWidth: 110 }}>Status</th>
                      <th className="th-right" style={{ minWidth: 110 }}>
                        Valor
                      </th>
                      ...
                      <tr key={l.id || i}>
                        ...
                        <td>
                          <button
                            ...
                            onClick={() =>
                              alert(
                                "Em breve: edição inline aqui (modal). Por ora o 'Realizado' no checkbox já funciona."
                              )
                            }
                          >
                            Alterar
